 ...
 */
export const formDataToCreateData = (formData: TripFormData): CreateTripData => {
  return {
    trip_title: formData.tripTitle,
    trip_date: formData.tripDate ? format(formData.tripDate, 'yyyy-MM-dd') : '',
    booking_start_time: buildDateTime(formData.bookingStartDate, formData.bookingStartTime),
    booking_end_time: buildDateTime(formData.bookingEndDate, formData.bookingEndTime),
